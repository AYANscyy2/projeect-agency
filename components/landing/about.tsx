      ...
      <div className="max-w-[1400px] mx-auto px-6 md:px-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-start">
          ...
          <div className="flex flex-col gap-8">
            <h2 className="font-anton text-[56px] md:text-[80px] uppercase text-primary leading-[0.9] reveal-text">
              WE ARE<br />BUILT FOR<br />BUILDERS.
            </h2>

            <div className="flex flex-col gap-5 border-l-4 border-primary pl-6">
              <p className="font-inter text-[18px] text-on-surface leading-relaxed reveal-text">
              ...
              <p className="font-inter text-[16px] text-on-surface-variant leading-relaxed reveal-text">
            ...
            {/* Mini Stats Row */}
            <div className="grid grid-cols-3 border-t-2 border-b-2 border-primary divide-x-2 divide-primary mt-4">
              {stats.map((s) => (
                <div key={s.label} className="flex flex-col items-center py-6 gap-1">
                  <span className="font-anton text-[40px] md:text-[48px] leading-none text-primary reveal-text">{s.value}</span>
                  <span className="font-inter text-[12px] uppercase tracking-[0.08em] text-on-surface-variant font-bold reveal-text">{s.label}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-10">
            ...
            <div className="relative w-full">
              <div className="absolute top-4 -left-4 lg:top-5 lg:-left-5 w-full h-full bg-primary z-0"></div>
            ...
            <div className="flex flex-col border-t-2 border-primary">
              {[
                ...
                <div key={v.num} className="flex gap-6 py-6 border-b-2 border-primary group cursor-default">
                  <span className="font-inter text-[13px] font-bold text-outline tracking-widest pt-1 shrink-0 reveal-text">{v.num}</span>
                  <div>
                    <h4 className="font-anton text-[22px] uppercase text-primary leading-none mb-2 group-hover:underline underline-offset-4 reveal-text">{v.title}</h4>
                    <p className="font-inter text-[15px] text-on-surface-variant leading-relaxed reveal-text">{v.body}</p>
                  </div>
                </div>
              ))}
            </div>
